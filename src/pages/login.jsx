...
const validationSchema = Yup.object().shape({
  ...
  password: Yup.string().required("Modpas obligatwa"),
})
    ...
    <div className="min-h-screen bg-[#f0faf7] flex flex-col items-center justify-center p-4">
      <Card className="w-full max-w-md bg-white p-8 rounded-xl shadow-sm">
        <div className="flex flex-col items-center space-y-2 mb-8">
          ...
          <h1 className="text-2xl font-semibold text-[#2d2d5f]">Konekte ak kont ou</h1>
          <p className="text-sm text-gray-500">Antre enfòmasyon ou pou w ka konekte.</p>
        </div>
        ...
        <form onSubmit={formik.handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <Link
                to="/chanje-modpas"
                className="text-sm text-[#40c4a7] hover:underline"
              >
                Bliye modpas?
              </Link>
            </div>
          ...
          >
            {formik.isSubmitting ? "Ap konekte..." : "Konekte"}
          </Button>
        </form>

        <p className="mt-6 text-center text-sm text-gray-500">
          Ou pa gen yon kont? {" "}
          <Link to="/anregistre" className="text-[#40c4a7] hover:underline">
            Kreye kont
          </Link>
        </p>
      </Card>
